...
async function fetchSolPrice(): Promise<number> {
  try {
    ...
    if (!res.ok) return _solPriceUsd;
    const data = await res.json();
    if (data?.priceUsd && !isNaN(data.priceUsd)) {
      _solPriceUsd = data.priceUsd;
    }
  } catch { /* use cached */ }
  return _solPriceUsd;
}
  ...
  const loadData = useCallback(async () => {
    ...
    if (connected && publicKey) {
      try {
        ...
        const totalUsd = (lamports / LAMPORTS_PER_SOL) * solPrice;
        ...
        appendLog({
          ...
          detail: `Live balance: ${(lamports / LAMPORTS_PER_SOL).toFixed(4)} SOL @ $${solPrice.toFixed(2)} = ~$${totalUsd.toFixed(2)}`,
        });
